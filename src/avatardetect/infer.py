...
def infer(config_path: str | Path, model_path: str | Path, prototypes_path: str | Path, image_path: str | Path, top_k: int) -> None:
    ...
    df = pd.read_csv(prototypes_path)
    ...
    for rank, idx in enumerate(order, start=1):
        ...
        print(
            ...
            f"character_id={row['character_id']} appearance_id={row['appearance_id']} "
            f"名称={row['character_name']} skin_id={row['skin_id']}"
        )
...
if __name__ == "__main__":
    args = parse_args()
    infer(args.config, args.model, args.prototypes, args.image, args.top_k)
