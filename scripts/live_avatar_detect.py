...
PREVIEW_WINDOW = "AvatarDetect Live"
MASK_WINDOW = "HSV Mask"
CONTROL_WINDOW = "AvatarDetect Control"
...
def draw_text_lines_top_right(
    ...
    for line in lines:
        ...
        text_x = max(0, min(image.width - text_w, x - text_w))
        text_y = max(0, min(image.height - text_h, y))
...
class LiveUi:
    def __init__(self) -> None:
        ...
        for row, (name, value) in enumerate(HSV_DEFAULTS.items()):
            if name.startswith("Area"):
                ...
                minimum, maximum = -80, 80
            elif name.startswith("Crop"):
                minimum, maximum = 50, 220
            else:
                minimum, maximum = 0, 255
            variable = tk.IntVar(value=value)
            self.variables[name] = variable
            tk.Label(self.root, text=name, width=12, anchor="w").grid(row=row, column=0, padx=6, pady=3, sticky="w")
            tk.Scale(
                self.root,
                from_=minimum,
                to=maximum,
                ...
                showvalue=True,
                resolution=1,
            ).grid(row=row, column=1, padx=6, pady=3, sticky="ew")
        ...
        for offset, (name, value) in enumerate(DISPLAY_DEFAULTS.items()):
            ...
            tk.Checkbutton(self.root, text=name, variable=variable, anchor="w").grid(
                row=display_start_row + offset,
                column=0,
                columnspan=2,
                padx=6,
                pady=3,
    ...
    def read_controls(self) -> tuple[np.ndarray, np.ndarray, int, int, int, int, int, int]:
        ...
        return (
            ...
            area_min,
            area_max,
            max(1, int(values["Crop W"])),
            max(1, int(values["Crop H"])),
            int(values["Crop X Offset"]),
            int(values["Crop Y Offset"]),
        )
    ...
    def show_mask(self, mask: np.ndarray) -> None:
        self.last_mask_bgr = cv2.cvtColor(mask, cv2.COLOR_GRAY2BGR) if mask.ndim == 2 else mask
...
def draw_mask_debug(
    ...
    for x, y, w, h, area in rects[:80]:
        ...
        cv2.rectangle(debug, (x, y), (x + w, y + h), color, 2 if in_range else 1)
        cv2.putText(debug, str(area), (x, max(12, y - 4)), cv2.FONT_HERSHEY_SIMPLEX, 0.45, color, 1, cv2.LINE_AA)
    return debug


def load_prototypes(path: Path) -> tuple[pd.DataFrame, np.ndarray]:
    prototypes = pd.read_csv(path, dtype=str).fillna("")
    required = {"embedding", "variant_id", "character_id", "character_name", "skin_id", "skin_name", "element_type"}
...
def avatar_label_lines(rows: list[tuple[pd.Series, float]]) -> list[str]:
    ...
    skin_name = str(row.get("skin_name", ""))
    element_type = str(row.get("element_type", ""))
    return [
        f"{row.get('character_name', '')}",
        f"{skin_name} {element_type}".strip(),
        f"Score: {score:.3f}",
        f"T2 Gap: {gap:.3f}",
    ]
...
def print_top5_snapshot(
    ...
    print("")
    print(
        f"===== Top5 snapshot rects={len(rects)} crop={crop_w}x{crop_h} "
        f"bottom_margin={CARD_BOTTOM_MARGIN} offset=({crop_offset_x},{crop_offset_y}) ====="
    )
    valid_count = 0
    for rect_index, rect in enumerate(rects, start=1):
        ...
        for position, (row, score) in enumerate(top_rows[:5], start=1):
            print(
                ...
                f"name={row.get('character_name', '')} "
                f"skin={row.get('skin_name', '')} "
                f"element={row.get('element_type', '')} "
...
def save_crops_snapshot(
    ...
    for rect_index, rect in enumerate(rects, start=1):
        ...
        gap = score - top2_score
        name = safe_filename_part(row.get("character_name", "unknown"))
        skin = safe_filename_part(row.get("skin_name", "") or "skin")
        element = safe_filename_part(row.get("element_type", "") or "element")
        filename = (
            f"{timestamp}_rect{rect_index:03d}_{name}_{skin}_{element}_"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="AvatarDetect 实机窗口识别测试")
    parser.add_argument("--config", default=str(ROOT / "configs" / "train.yaml"))
    parser.add_argument("--model", default=str(ROOT / "outputs" / "avatar.onnx"))
...
def main() -> int:
    ...
    title = ""
    last_search = 0.0
    result_cache: dict[tuple[int, int, int, int], tuple[float, list[tuple[pd.Series, float]]]] = {}
        ...
        if hwnd is None:
            ...
            if frame_rgb is None:
                ...
                for rect in rects:
                    ...
                    cache_key = (round(crop_x1 / CACHE_GRID_PX), round(crop_y1 / CACHE_GRID_PX), crop_w, crop_h)
                ...
                status = (
                    ...
                    f"area={area_min}-{area_max}  crop={crop_w}x{crop_h}  "
                    f"bottom_margin={CARD_BOTTOM_MARGIN}  offset={crop_offset_x},{crop_offset_y}"
                )
